...
pinModel = pin.buildModelFromUrdf(
    "/home/pshah/Applications/atlas_urdf/robot.urdf", pin.JointModelFreeFlyer()
)
...
q0 = np.array(
    [
        ...
        0.0,
        0.0,
        0.0,  # left hip abductors
        0.0,
        0.0,
        0.0,  # left knee, ankle tilt fwd, ankle tilt side
        0.0,
        0.0,
        0.0,  # right hip abductors
        0.0,
        0.0,
        0.0,
    ]
)  # right knee, right ankle tilt fwd, right ankle tilt side;
...
des_vel_left = np.tile(np.array([0.0, 0, 0]), (int(T / dt), 1))
des_vel_right = np.tile(np.array([0.0, -0, 0]), (int(T / dt), 1))

des_com_pos = np.tile(np.array([0.0, 0, 1.2]), (int(T / dt), 1))
...
gg.create_swing_foot_task(
    des_pos_left[0], des_pos_left[0] + [0.2, 0, 0], 0, 0.4, 0.1, "l_foot", "L_step", 1e3
)
gg.create_contact_task(des_pos_left[0], 0, T, "r_foot", "R_step", 1e5)
gg.create_contact_task(des_pos_right[0], 0, T, "l_foot", "L_step", 1e5)
