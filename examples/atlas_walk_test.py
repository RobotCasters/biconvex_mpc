...
walk.stance_percent = n_eff * [
    0.6,
]
walk.gait_dt = 0.01
walk.phase_offset = int(0.5 * n_eff) * [0.0,] + int(0.5 * n_eff) * [
    0.5,
]
...
walk.state_wt = np.array(
    ...
    + [0.00] * 3
    + [100] * 3
    + [0.1] * (pin_robot.model.nv - 6)
)

walk.ctrl_wt = [0, 0, 1] + [1, 1, 1] + [5.0] * (rmodel.nv - 6)

walk.swing_wt = [1e5, 2e5]
walk.cent_wt = [5e1, 1e2]
walk.step_ht = 0.3
walk.nom_ht = 1.12
walk.reg_wt = [5e-2, 1e-5]

# Dyn
walk.W_X = np.array([1e0, 1e0, 1e4, 1e-4, 1e-4, 2e0, 3e-1, 1e-1, 3e-1])
walk.W_X_ter = 10.0 * np.array([1e0, 1e0, 1e6, 1e-1, 1e-1, 2e3, 1e-1, 1e-1, 1e-1])
...
xs_plan, us_plan, f_plan = gg.optimize(q, v, 0.0, v_des, w_des)
