...
jump.gait_dt = 0.05
jump.phase_offset = np.array(8 * [0.0])
...
jump.cent_wt = [1.0, 5e2]
jump.step_ht = 0.1
jump.nom_ht = 1.12
jump.reg_wt = [5e-2, 1e-5]

# Dyn
jump.W_X = np.array([1e-5, 1e-5, 1e5, 1e1, 1e1, 2e2, 1e4, 2e5, 1e4])
jump.W_X_ter = 10 * np.array([1e5, 1e-5, 1e5, 1e1, 1e1, 2e2, 1e5, 2e5, 1e5])
jump.W_F = np.array(4 * [1e1, 1e1, 1.5e1])
jump.rho = 5e4
