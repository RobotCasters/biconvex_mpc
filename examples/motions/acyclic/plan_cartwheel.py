...
plan.cnt_plan = [
    ...
    [
        [1.0, 0.3946, 0.14695, 0.0, st, st + flip_time],
        [1.0, 0.3946, -0.14695, 0.0, st, st + flip_time],
        [0.0, 0.0054, 0.14695, 0.0, st, st + flip_time],
        [0.0, 0.0054, -0.14695, 0.0, st, st + flip_time],
    ],
    [
        [1.0, 0.3946, 0.14695, 0.0, st + flip_time, T],
        [1.0, 0.3946, -0.14695, 0.0, st + flip_time, T],
        [1.0, 0.8054, 0.14695, 0.0, st + flip_time, T],
        [1.0, 0.8054, -0.14695, 0.0, st + flip_time, T],
    ],
]

plan.n_col = int(np.round(T / dt))
plan.dt_arr = (plan.n_col + 1) * [
    dt,
]
plan.plan_freq = [[0.6, 0, T], [1.0, T, T + 1.5]]
#  dynamic optimization params
plan.W_X = np.array([1e-2, 1e-2, 1e5, 1e-2, 1e-2, 1e-4, 1e3, 1e3, 1e4])
plan.W_X_ter = 10 * np.array([1e-2, 1e-2, 1e5, 1e-2, 1e-2, 1e-4, 1e3, 1e4, 1e4])
plan.W_F = np.array(4 * [1e1, 1e1, 2e0])
plan.rho = 5e4

plan.X_nom = [
    [0.2, 0, 0.2, 0, 0, 0, 0, 0.1, 0.0, 0, st],
    [0.4, 0, 0.3, 0, 0, 0, 0, 0.6, 0.0, st, st + flip_time],
    [0.6, 0, 0.2, 0, 0, 0, 0, 0.0, 0.0, st + flip_time, T],
]

plan.X_ter = [0.2, 0, 0.2, 0, 0, 0, 0, 0.0, 0.0]

plan.bounds = [
    [-0.45, -0.45, 0.0, 0.45, 0.45, 0.3, 0, st],
...
plan.state_reg = [
    np.hstack((x_reg1, [0, st + flip_time])),
    np.hstack((x_reg2, [st + flip_time, T])),
]
plan.state_wt = [
    np.hstack((state_wt_1, [0, st + flip_time])),
    np.hstack((state_wt_2, [st + flip_time, T])),
]
