...
# IK
still.state_wt = np.array(
    [0.0, 0, 10]
    + [1000] * 3
    + [1.0] * (pin_robot.model.nv - 6)
    + [0.00] * 3
    + [100] * 3
...
still.cent_wt = [0 * 5e1, 5e2]
still.step_ht = 0.13
still.nom_ht = 0.26
still.reg_wt = [5e-2, 1e-5]

# Dyn
still.W_X = np.array([1e-5, 1e-5, 1e5, 1e1, 1e1, 2e2, 1e4, 1e4, 1e4])
still.W_X_ter = 10 * np.array([1e5, 1e5, 1e5, 1e1, 1e1, 2e2, 1e5, 1e5, 1e5])
still.W_F = np.array(4 * [1e1, 1e1, 1e1])
still.rho = 5e4
...
gallop.cent_wt = [5e3, 5e3]
gallop.step_ht = 0.08
gallop.nom_ht = 0.26
gallop.reg_wt = [5e-2, 1e-5]

# Dyn
gallop.W_X = np.array([1e-5, 1e-5, 1e5, 1e1, 1e1, 2e2, 1e5, 1e5, 1e5])
gallop.W_X_ter = 10 * np.array([1e5, 1e5, 1e5, 1e1, 1e1, 2e2, 1e5, 1e5, 1e5])
gallop.W_F = np.array(4 * [1e1, 1e1, 1e1])
gallop.rho = 5e4
...
# IK
walk.state_wt = np.array(
    [0.0, 0, 1000]
    + [1e3] * 3
    + [0.5] * (pin_robot.model.nv - 6)
    + [0.00] * 3
    + [50] * 3
    + [1e-2] * (pin_robot.model.nv - 6)
)
...
# Dyn
walk.W_X = np.array([1e-5, 1e-5, 1e5, 1e2, 1e2, 1e2, 5e3, 5e3, 5e3])
walk.W_X_ter = 10 * np.array([1e-5, 1e-5, 1e5, 1e2, 1e2, 1e2, 1e3, 1e3, 1e3])
walk.W_F = np.array(4 * [1e1, 1e1, 1e1])
walk.nom_ht = 0.24
walk.rho = 5e4
