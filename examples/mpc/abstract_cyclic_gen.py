...
class SoloMpcGaitGen:
    def __init__(self, robot, r_urdf, x_reg, planning_time, q0, height_map=None):
        ...
        for i in range(len(self.eff_names)):
            ...
            self.offsets[i] = (
                self.rdata.oMf[self.rmodel.getFrameId(self.hip_names[i])].translation
                - com_init.copy()
            )
    ...
    def plot_plan(self, q, v, plot_force=True):
        ...
        if plot_force:
            ...
            for n in range(self.n_eff):
                ax_f[n].plot(
                    F_opt[3 * n :: 3 * self.n_eff], label=self.eff_names[n] + " Fx"
                )
                ax_f[n].plot(
                    F_opt[3 * n + 1 :: 3 * self.n_eff], label=self.eff_names[n] + " Fy"
                )
                ax_f[n].plot(
                    F_opt[3 * n + 2 :: 3 * self.n_eff], label=self.eff_names[n] + " Fz"
                )
                ax_f[n].grid()
                ax_f[n].legend()
