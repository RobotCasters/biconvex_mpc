    ...
    def create_costs(self, q, v, t, make_cyclic=False):
        ...
        if self.params.use_offline_traj:
            if int(t / self.params.dt) < len(self.X_centroidal_offline[0::9]):
                if int(t / self.params.dt) + self.horizon <= len(
                    self.X_centroidal_offline[0::9]
                ):
                    X_nom[0 : 9 * self.horizon] = self.X_centroidal_offline[
                        9
                        * (int(t / self.params.dt)) : 9
                        * (int(t / self.params.dt) + self.horizon)
                    ]
                else:
                    X_nom[
                        0 : len(self.X_centroidal_offline) - 9 * int(t / self.params.dt)
                    ] = self.X_centroidal_offline[9 * (int(t / self.params.dt)) :]
                    for i in range(
                        len(self.X_centroidal_offline[0::9]) - int(t / self.params.dt),
                        self.horizon,
                    ):
                        X_nom[9 * i : 9 * (i + 1)] = self.X_centroidal_offline[-9:]
            else:
                for i in range(self.horizon):
                    X_nom[9 * i : 9 * (i + 1)] = 0.0 * self.X_centroidal_offline[-9:]
                    X_nom[9 * i + 2] = 0.9
            X_ter = X_nom[-9:]
        else:
            ...
            while i < self.params.n_col:
                ...
                if ft < self.params.X_nom[-1][-1]:
                    for k in range(len(self.params.X_nom)):
                        if (
                            ft >= self.params.X_nom[k][9]
                            and ft < self.params.X_nom[k][10]
                        ):
        ...
        # ax[0].plot(X_nom[0::9], label="Dyn com x")
        # ax[1].plot(X_nom[1::9], label="Dyn com y")
        # ax[2].plot(X_nom[2::9], label="Dyn com z")
        # ax[0].legend()
        # fig, ax = plt.subplots(6,1)
        # ax[0].plot(X_nom[3::9], label="lmom x")
        # ax[1].plot(X_nom[4::9], label="lmom y")
        # ax[2].plot(X_nom[5::9], label="lmom z")
        # ax[3].plot(X_nom[6::9], label="amom x")
        # ax[4].plot(X_nom[7::9], label="amom y")
        # ax[5].plot(X_nom[8::9], label="amom z")
        # ax[0].legend()
        # plt.show()
                    ...
                    if int(t / self.params.dt) + self.horizon <= len(
                        ...
                        if i < self.horizon:
                            ...
                            self.ik.add_state_regularization_cost_single(
                                i,
                                self.params.state_scale[0][0],
                                "xReg",
                                self.params.state_wt[0][0 : 2 * self.rmodel.nv],
                        ...
                        if i < self.horizon:
                            ...
                            if i < len(self.X_kinematics_offline[:][:]) - int(
                                t / self.params.dt
                            ):
                                self.ik.add_state_regularization_cost_single(
                                    i,
