...
class DataRecorder:
    ...
    def record_data(self, q, v, tau, f, des_q, des_v, des_a, des_f):
        ...
        self.des_com.append(
            pin.centerOfMass(self.rmodel, self.rdata, des_q.copy(), des_v.copy())
        )
    ...
    def plot_plans(self):
        ...
        t_arr = 0.001 * np.arange(
            1, 1000 * self.record_time[-1] + len(self.xs_plan[-1][:, 0]) + 1
        )
        ...
        for i in range(len(self.xs_plan)):
            ...
            ax[0].plot(
                t_arr[st : st + len(self.xs_plan[i][:, 0])],
                self.xs_plan[i][:, 0],
                label="plan base x - " + str(0.001 * st) + " sec",
            )
            ax[1].plot(
                t_arr[st : st + len(self.xs_plan[i][:, 1])],
                self.xs_plan[i][:, 1],
                label="plan base y - " + str(0.001 * st) + " sec",
            )
            ax[2].plot(
                t_arr[st : st + len(self.xs_plan[i][:, 2])],
                self.xs_plan[i][:, 2],
                label="plan base z - " + str(0.001 * st) + " sec",
            )
        ...
        t_arr = 0.001 * np.arange(
            1, 1000 * self.record_time[-1] + len(self.f_plan[-1][:, 0]) + 1
        )
        for i in range(len(self.f_plan)):
            for n in range(4):
                ...
                ax_f[n].plot(
                    t_arr[st : st + len(self.f_plan[i][:, 3 * n])],
                    self.f_plan[i][:, 3 * n + 2],
                    label=eff_names[n] + " Fz",
                )
        ...
        for n in range(4):
            ...
            ax_f[n].plot(
                t_arr[0 : len(F_arr[:, 3 * n])],
                F_arr[:, 3 * n + 2],
                label="real " + eff_names[n] + " Fz",
            )
