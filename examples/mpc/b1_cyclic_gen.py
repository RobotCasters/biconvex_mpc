...
class B1MpcGaitGen:
    def __init__(self, robot, r_urdf, x_reg, planning_time, q0, height_map=None):
        ...
        self.eff_names = ["LF_FOOT", "LH_FOOT", "RF_FOOT", "RH_FOOT"]
        self.hip_names = ["LF_HFE", "LH_HFE", "RF_HFE", "RH_HFE"]
        self.n_eff = 4

        pin.forwardKinematics(self.rmodel, self.rdata, q0, np.zeros(self.rmodel.nv))
        pin.updateFramePlacements(self.rmodel, self.rdata)
        com_init = pin.centerOfMass(
            self.rmodel, self.rdata, q0, np.zeros(self.rmodel.nv)
        )
        ...
        for i in range(len(self.eff_names)):
            ...
            self.offsets[i] = (
                self.rdata.oMf[self.rmodel.getFrameId(self.eff_names[i])].translation
                - com_init.copy()
            )
        ...
        self.offsets[0][1] -= 0.00  # Front Left_Y

        self.offsets[1][0] -= 0.00  ##Hind Left_X
        self.offsets[1][1] += 0.00  # Hind Left_Y

        self.offsets[2][0] += 0.00  # Front Right_X
        self.offsets[2][1] += 0.00  # Front Right_Y

        self.offsets[3][0] -= 0.00  # Hind Right X
        self.offsets[3][1] += 0.00  # Hind Right Y
    ...
    def update_gait_params(self, weight_abstract, t, ik_hor_ratio=0.5):
        ...
        self.gait_planner = GaitPlanner(
            ...
            np.array(self.params.stance_percent),
            np.array(self.params.phase_offset),
            self.params.step_ht,
        )
        ...
        self.size = min(
            self.ik_horizon, int(self.planning_time / self.params.gait_dt) + 2
        )
    ...
    def create_cnt_plan(self, q, v, t, v_des, w_des):
        ...
        vtrack = v_des[
            0:2
        ]  # this effects the step location (if set to vcom it becomes raibert)
        # vtrack = vcom[0:2]
        ...
        for i in range(self.horizon):
            for j in range(len(self.eff_names)):
                if i == 0:
                    if self.gait_planner.get_phase(t, j) == 1:
                        ...
                        self.cnt_plan[i][j][1:4] = np.round(
                            self.rdata.oMf[self.ee_frame_id[j]].translation, 3
                        )
                    ...
                    if self.gait_planner.get_phase(ft, j) == 1:
                        ...
                        if self.cnt_plan[i - 1][j][0] == 1:
                            ...
                            hip_loc = (
                                com
                                + np.matmul(R, self.offsets[j])[0:2]
                                + i * self.params.gait_dt * vtrack
                            )
                            raibert_step = (
                                0.5
                                * vtrack
                                * self.params.gait_period
                                * self.params.stance_percent[j]
                                - 0.05 * (vtrack - v_des[0:2])
                            )
                            ang_step = 0.5 * np.sqrt(z_height / self.gravity) * vtrack
                            ang_step = np.cross(ang_step, [0.0, 0.0, w_des])

                            self.cnt_plan[i][j][1:3] = (
                                raibert_step[0:2] + hip_loc + ang_step[0:2]
                            )

                            if self.height_map != None:
                                self.cnt_plan[i][j][3] = (
                                    self.height_map.getHeight(
                                        self.cnt_plan[i][j][1], self.cnt_plan[i][j][2]
                                    )
                                    + self.foot_size
                                )
                            else:
                        ...
                        self.cnt_plan[i][j][0] = 0
                        per_ph = np.round(
                            self.gait_planner.get_percent_in_phase(ft, j), 3
                        )
                        hip_loc = (
                            com
                            + np.matmul(R, self.offsets[j])[0:2]
            ...
            if i == 0:
                dt = self.params.gait_dt - np.round(
                    np.remainder(t, self.params.gait_dt), 2
                )
                if dt == 0:
    ...
    def create_costs(self, q, v, v_des, w_des, ori_des):
            ...
            for j in range(len(self.eff_names)):
                if self.cnt_plan[i][j][0] == 1:
                    ...
                    self.ik.add_position_tracking_task_single(
                        self.ee_frame_id[j],
                        pos,
                        self.params.swing_wt[1],
                        "via_" + str(0) + self.eff_names[j],
                        i,
                    )
        ...
        self.ik.add_state_regularization_cost(
            0,
            self.ik_horizon,
            self.params.reg_wt[0],
            "xReg",
            self.params.state_wt,
            self.x_reg,
            False,
        )
        ...
        for i in range(1, self.horizon):
            self.X_nom[9 * i + 0] = (
                self.X_nom[9 * (i - 1) + 0] + v_des[0] * self.dt_arr[i]
            )
            self.X_nom[9 * i + 1] = (
                self.X_nom[9 * (i - 1) + 1] + v_des[1] * self.dt_arr[i]
            )
        ...
        # Compute angular momentum (amom) / orientation correction
        ...
        X_ter[0:2] = (
            self.X_init[0:2]
            + (self.params.gait_horizon * self.params.gait_period * v_des)[0:2]
        )  # Changed this
        ...
        for i in range(self.size):
            if i == 0:
                self.f_int = np.linspace(
                    F_opt[i * n_eff : n_eff * (i + 1)],
                    F_opt[n_eff * (i + 1) : n_eff * (i + 2)],
                ...
                self.com_int = np.linspace(
                    com_opt[i], com_opt[i + 1], int(self.dt_arr[i] / 0.001)
                )
                self.mom_int = np.linspace(
                    mom_opt[i], mom_opt[i + 1], int(self.dt_arr[i] / 0.001)
                )
            else:
                ...
                self.xs_int = np.vstack(
                    (
                        self.xs_int,
                        np.linspace(xs[i], xs[i + 1], int(self.dt_arr[i] / 0.001)),
                    )
                )
                self.us_int = np.vstack(
                    (
                        self.us_int,
                        np.linspace(us[i], us[i + 1], int(self.dt_arr[i] / 0.001)),
                    )
                )

                self.com_int = np.vstack(
                    (
                        self.com_int,
                        np.linspace(
                            com_opt[i], com_opt[i + 1], int(self.dt_arr[i] / 0.001)
    ...
    def plot(self, q, v, plot_force=True):
        ...
        if plot_force:
            ...
            for n in range(self.n_eff):
                ax_f[n].plot(
                    optimized_forces[3 * n :: 3 * self.n_eff],
                    label=self.eff_names[n] + " Fx",
                )
                ax_f[n].plot(
                    optimized_forces[3 * n + 1 :: 3 * self.n_eff],
                    label=self.eff_names[n] + " Fy",
                )
                ax_f[n].plot(
                    optimized_forces[3 * n + 2 :: 3 * self.n_eff],
    ...
    def plot_joints(self):
        ...
        for i in range(len(self.xs_traj)):
            ...
            ax[0].plot(
                x[st_hor : st_hor + len(self.xs_traj[i])], self.xs_traj[i][:, 10]
            )
    ...
    def plot_plan(self, q, v, plot_force=True):
        ...
        if plot_force:
            ...
            for n in range(self.n_eff):
                ax_f[n].plot(
                    t_arr[:-1],
                    F_opt[3 * n :: 3 * self.n_eff],
                    label=self.eff_names[n] + " Fx",
                )
                ax_f[n].plot(
                    t_arr[:-1],
                    F_opt[3 * n + 1 :: 3 * self.n_eff],
                    label=self.eff_names[n] + " Fy",
                )
                ax_f[n].plot(
                    t_arr[:-1],
                    F_opt[3 * n + 2 :: 3 * self.n_eff],
                    label=self.eff_names[n] + " Fz",
                )
                ax_f[n].grid()
                ax_f[n].legend()
