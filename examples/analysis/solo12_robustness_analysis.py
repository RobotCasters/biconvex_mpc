...
from solo12_gait_params import (
    trot,
    walk,
    air_bound,
    bound,
    ...
    for o in range(int(105000 * (plan_freq / sim_dt))):
        ...
        if pln_ctr == 0:
            ...
            xs_plan, us_plan, f_plan = gg.optimize(
                q, v, np.round(sim_t, 3), v_des, w_des
            )
        ...
        if push_index <= push_duration:
            push_index += 1
            robot_env.robot.rai_robot.setExternalForce(
                0,
                np.array([0, 0, 0]),
                np.array(
                    [
                        magnitude * math.cos(direction[dir_idx]),
                        magnitude * math.sin(direction[dir_idx]),
                        0,
                    ]
                ),
            )
