...
from robot_properties_bolt.bolt_humanoid_wrapper import (
    BoltHumanoidRobot,
    BoltHumanoidConfig,
)
...
if plan.use_offline_traj:
    ...
    mg.update_motion_params(plan, q, 0.0)
    mg.optimize(q, v, 0.0)
    ...
    plan.plan_freq = [[0.1, 0.0, plan.T]]
    plan.W_X = np.array([1e-5, 1e-5, 1e-5, 1e-4, 1e-4, 2e3, 3e4, 3e4, 3e4])
    plan.W_X_ter = 0.0 * np.array([1e5, 1e5, 1e5, 1e-1, 1e-1, 2e2, 1e5, 1e5, 1e5])
    ...
    plan.cent_wt = [[500.0, 10.0, 500.0], [0.1, 0.1, 5.0, 0.1, 100.0, 0.1]]
    plan.cnt_wt = 5e4
